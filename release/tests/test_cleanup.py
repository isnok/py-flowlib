...
from flowtool_releasing.cleanup import local_tag_cleanup
...
def used_project(permanent_repo):
...
def test_no_versioning(nogit):
    ...
    result = runner.invoke(local_tag_cleanup, ['--', '-20'])
    assert result.exit_code == 1
    assert 'Negative n supplied:' in result.output

    result = runner.invoke(local_tag_cleanup, [nogit])
    assert result.exit_code == 2
    assert 'Invalid value for ' in result.output


def test_including_deploy(used_project):

    repo_root = os.path.dirname(used_project.git_dir)
